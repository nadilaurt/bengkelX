function pass = password(charset)
...
switch data.Key
   ...
   case 'return'
      uiresume
      conn = database('login','','');
      query_id   = 'SELECT username FROM login';
      query_pass = 'SELECT password FROM login';
      data_id = table2array(fetch(conn,query_id));
      data = table2array(fetch(conn,query_pass));
      if any(strcmp(pass,data))
         form_penjualan;
         close(form_login);
      else
          errordlg('Masukkan password yang sesuai','Password Salah','modal');
          close(conn);
      end
      return
      %
